...
from src.universe import VN30_SYMBOLS
...
def build_cached_breadth():
    ...
    adv = float((last[valid] > prev[valid]).mean() * 100)
    ...
    effective = float(1 / hhi) if hhi > 0 else np.nan
    top5 = float(weights.nlargest(5).sum() * 100)

    details = pd.DataFrame({
        'Mã': panel.columns,
        'Trên MA20': ['Có' if last[s] > ma20[s] else 'Không' for s in panel.columns],
        'Trên MA50': ['Có' if last[s] > ma50[s] else 'Không' for s in panel.columns],
        'Trên MA200': ['Có' if last[s] > ma200[s] else 'Không' for s in panel.columns],
        'Tăng phiên gần nhất': ['Có' if last[s] > prev[s] else 'Không' for s in panel.columns]
    })
    risk_table = pd.DataFrame({
        'Mã': weights.index,
        'Biến động 20 phiên %': (vol * 100).round(2),
        'Tỷ trọng biến động %': (weights * 100).round(2)
    }).sort_values('Tỷ trọng biến động %', ascending=False)

    return {
        'date': panel.index.max(),
        'symbols': len(SYMBOLS),
        'valid_symbols': len(prices),
        'failed': failed,
        'source': 'Dữ liệu nền đã lưu',
        'pct_above_ma20': p20,
        'pct_above_ma50': p50,
        ...
        'dispersion_percentile': q,
        'dispersion_state': dispersion_state,
        'effective_risk_names': effective,
        'top5_risk_share': top5,
        'concentration_percentile': np.nan,
        'concentration_state': 'CHƯA ĐÁNH GIÁ LỊCH SỬ',
