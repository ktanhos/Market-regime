...
def fetch_history(fetcher, start_date, end_date):
    try:
        return normalize_ohlcv(fetcher(start=start_date.strftime("%Y-%m-%d"), end=end_date.strftime("%Y-%m-%d"), interval="1D"))
    except Exception:
        pieces, cursor, end_ts = [], pd.Timestamp(start_date), pd.Timestamp(end_date)
        while cursor <= end_ts:
            chunk_end = min(cursor + pd.Timedelta(days=119), end_ts)
            pieces.append(normalize_ohlcv(fetcher(start=cursor.strftime("%Y-%m-%d"), end=chunk_end.strftime("%Y-%m-%d"), interval="1D")))
...
def fetch_index_history(market, symbol, start_date, end_date):
    return fetch_history(market.index(symbol).ohlcv, start_date, end_date)


def fetch_equity_history(market, symbol, start_date, end_date):
    return fetch_history(market.equity(symbol).ohlcv, start_date, end_date)
...
def build_breadth(market, start_date, end_date):
    ...
    for i, symbol in enumerate(symbols):
        try:
            df = fetch_equity_history(market, symbol, breadth_start, end_date)
            series[symbol] = df.set_index("date")["close"].rename(symbol)
        except Exception as exc:
            failed.append(f"{symbol}: {type(exc).__name__}")
        progress.progress((i + 1) / len(symbols), text=f"Breadth {i + 1}/{len(symbols)}: {symbol}")
    progress.empty()
    if len(series) < 10:
    ...
    breadth["pct_above_ma20"] = ((prices > ma20) & valid).sum(axis=1) / coverage * 100
    breadth["pct_above_ma50"] = ((prices > ma50) & valid).sum(axis=1) / coverage * 100
    breadth["pct_above_ma200"] = ((prices > ma200) & valid).sum(axis=1) / coverage * 100
    ...
    return breadth.reset_index().rename(columns={"index": "date"}), {"symbols": len(series), "failed": failed, "source": source, "start": breadth_start}
...
def classify_regime(row):
    ...
    if pd.notna(breadth):
        if row["roro"] > 0 and breadth in ["RẤT RỘNG", "RỘNG"] and row["stress_z"] < 0.5:
            return "EXPANSION"
        if row["roro"] > 0 and (breadth in ["HẸP", "RẤT HẸP"] or row["stress_z"] >= 0.5):
            return "FRAGILE RALLY"
        if row["roro"] <= 0 and breadth == "RẤT HẸP" and row["stress_z"] >= 1.5:
            return "HIGH STRESS RISK OFF"
        if row["roro"] <= 0 and breadth in ["HẸP", "RẤT HẸP"]:
            return "CORRECTION"
        return "TRANSITION"
    if row["roro"] > 0 and row["stress_z"] < 0.5:
        return "EXPANSION"
    if row["roro"] > 0:
        return "FRAGILE RALLY"
    if row["stress_z"] >= 1.5:
...
def regime_validation(df):
    ...
    for horizon in [5, 10, 20]:
        test[f"forward_{horizon}"] = (test["close"].shift(-horizon) / test["close"] - 1) * 100
    rows = []
    for regime, group in test.groupby("regime"):
        if regime == "CHƯA ĐỦ DỮ LIỆU":
            continue
...
def show_market(name, df):
    ...
    with tabs[1]:
        ...
        st.caption("RORO dương là Risk On. RORO âm là Risk Off.")
    with tabs[2]:
        st.line_chart(chart[["parkinson_vol", "stress_score"]].dropna(how="all"), use_container_width=True)
        st.caption("Stress là thước đo tương đối so với lịch sử, không phải xác suất giảm giá.")
    with tabs[3]:
        cols = [c for c in ["pct_above_ma20", "pct_above_ma50", "pct_above_ma200", "pct_advancers", "breadth_score"] if c in chart.columns]
        if cols:
            st.line_chart(chart[cols].dropna(how="all"), use_container_width=True)
            st.caption("Breadth hiện dùng thành phần VN30 hiện tại và tối đa ba năm dữ liệu để giảm sai lệch sống sót. Chưa phải Breadth toàn thị trường.")
        else:
            st.info("Chưa có dữ liệu Breadth.")
    with tabs[4]:
        st.dataframe(regime_validation(df).round(4), use_container_width=True, hide_index=True)
        st.caption("T5, T10, T20 là lợi suất trung bình phần trăm sau 5, 10, 20 phiên. Tỷ lệ dương cũng tính bằng phần trăm. Đây là kiểm định mô tả trên cùng tập dữ liệu.")


st.title("Vietnam Market Regime")
st.caption("Khung phân tích chế độ thị trường Việt Nam")
st.write("Trend và Stress đã có thể quan sát và kiểm định mô tả. Breadth được bổ sung ở lớp nghiên cứu tiếp theo. Regime hiện tại là phân loại nghiên cứu, chưa phải tín hiệu giao dịch.")
...
if run:
    ...
    try:
        with st.status("Đang kết nối và phân tích", expanded=True) as status:
            ...
            breadth, meta = None, None
            if enable_breadth:
                st.write("Đang xây Breadth từ VN30")
                breadth, meta = build_breadth(market, start_date, end_date)
                st.write(f"Breadth dùng {meta['symbols']} mã, từ {meta['start']}")
                if meta["failed"]:
                    st.warning(f"Không lấy được {len(meta['failed'])} mã")
...
if results:
    ...
    if meta:
        st.info(f"Breadth: {meta['source']}. Số mã sử dụng: {meta['symbols']}. Bắt đầu từ {meta['start']}.")
    for name in ["VNINDEX", "VN30"]:
        show_market(name, results[name])
    st.divider()
    st.header("Lộ trình mô hình")
    st.write("Trend và Stress đã có thể quan sát và kiểm định mô tả. Breadth hiện được xây thử nghiệm bằng dữ liệu VN30. Bước tiếp theo là mở rộng Breadth sang HOSE và kiểm định ngoài mẫu trước khi cố định Regime.")
else:
